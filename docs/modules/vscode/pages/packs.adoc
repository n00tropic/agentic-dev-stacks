...
== Post-import helper (profile assets)

After importing a profile, preview/apply tasks, launch configs, snippets, keybindings, and MCP TOML for a slug:
...
Use `--apply --target-dir <workspace>` to copy `.vscode` assets into another workspace; the script prints MCP TOML to paste into `~/.codex/config.toml`.
