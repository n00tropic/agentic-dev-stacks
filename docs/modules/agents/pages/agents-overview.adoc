...
stateDiagram-v2
    ...
    Planning --> EditPacks: Modify Profile
    Planning --> ValidateConfig: Check Config
    
    EditPacks --> ValidateExtensions: Run Validators
    ValidateExtensions --> ValidateMCP: Check MCP Manifests
    ValidateMCP --> ExportWorkspace: Generate Exports
    
    ExportWorkspace --> BuildBundle: Create Bundle
    BuildBundle --> TestBundle: Validate Bundle
    
    TestBundle --> UpdateDocs: Document Changes
    UpdateDocs --> CodeReview: Request Review
    
    CodeReview --> FixIssues: Issues Found
    FixIssues --> TestBundle
    CodeReview --> Complete: Approved
    
    ValidateConfig --> UpdateDocs
    Complete --> [*]
    
    note right of EditPacks
        Never edit user dotfiles
        Update CONTROL.md if needed
    end note
    
    note right of ValidateMCP
        Check for secrets
        Verify placeholders
    end note
....
...
State machine showing the standard workflow for agents making changes to profiles and configuration.
...
- **Dist profiles:** regenerate via VS Code **Export Profile…**, save to `vscode/profiles-dist/<slug>.code-profile`, optionally publish to secret gist and update `PROFILE_DIST.md`.
