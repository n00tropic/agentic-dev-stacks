!(function () {
      ...
      if (n)
        for (
          ...
          if (
            (c =
              c ||
              ((c = l.test(i.className)) && (i.firstElementChild || {}).id)) &&
  ...
  (i
    ...
    g(o, ".nav-item-toggle").forEach(function (e) {
      ...
      e &&
        ((e.style.cursor = "pointer"), e.addEventListener("click", u.bind(t)));
    }),
    e &&
      ...
      e.addEventListener("click", function () {
        ...
        (g(o, ".nav-item > .nav-item-toggle").forEach(function (e) {
          ...
          s
            ? (t && d(s), p(o, s.querySelector(".nav-link")))
            : (o.scrollTop = 0));
      })),
    t &&
  ...
  function p() {
    ...
    e && window.innerHeight + e + 2 >= document.documentElement.scrollHeight
      ...
      : (Array.isArray(a) &&
          ...
          (a = undefined)),
        s.some(function (e) {
          if (e.getBoundingClientRect().top + h(e, "paddingTop") - n > r)
            return !0;
          t = "#" + e.id;
        }),
        t
      ...
      function (t) {
        ...
        );
        {
          ...
          t.classList.toggle("is-active") &&
            ((t.style.maxHeight = ""),
            (e =
              window.innerHeight - Math.round(t.getBoundingClientRect().top)),
          ...
          "svg" === r
            ? ((a = document.createElementNS(
              ...
              (c = document.createElementNS(
                "http://www.w3.org/2000/svg",
                "use",
              )).setAttribute("href", p + "/img/octicons-16.svg#icon-clippy"),
              a.appendChild(c),
              n.appendChild(a))
            : (((c = document.createElement("img")).src =
                p + "/img/octicons-16.svg#view-clippy"),
              (c.alt = "copy icon"),
...
!(function () {
  ...
  if (navTrigger) {
    var navTarget = document.getElementById(navTrigger.dataset.target);
    navTarget &&
      (navTrigger.addEventListener("click", function (event) {
        event.stopPropagation();
        var open = navTarget.classList.toggle("is-open");
        navTrigger.setAttribute("aria-expanded", open);
      }),
      document.addEventListener("click", function (event) {
        navTarget.contains(event.target) ||
  ...
  var STORAGE_KEY = "ads-theme";
  var prefersDark = window.matchMedia("(prefers-color-scheme: dark)");

  function applyTheme(next) {
    docEl.setAttribute("data-theme", next);
    themeToggle && themeToggle.setAttribute("aria-pressed", next === "light");
    try {
  ...
  function initialTheme() {
    ...
    return prefersDark.matches ? "dark" : "dark";
  }
  ...
  function toggleTheme() {
    ...
    themeToggle.classList.add("is-peeking");
    setTimeout(function () {
      themeToggle.classList.remove("is-peeking");
    }, 320);
  }
