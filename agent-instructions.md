...
- Dist profile exports (`vscode/profiles-dist/*.code-profile`) plus mapping in `PROFILE_DIST.md` (optional secret gists for one-click import).
- Validation and merge scripts (`export-packs.py`, `merge-mcp-fragments.py`, helpers).
- A vendor-neutral setup that still supports GitHub/Copilot when requested (works offline/local by default).
...
5. For GitHub-specific flows (Copilot, GitHub MCP, secret gists), keep support intact and cross-check `copilot-instructions.md`.
